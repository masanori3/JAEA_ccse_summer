...
model = Chain(Dense(k,10,relu),Dense(10,10,relu),Dense(10,10,sigmoid),Dense(10,1))
# check "W" and " b"
...
function train_batch!(xtest,ytest,model,loss,opt,nt)
    for it=1:nt
        ...
        if it% 100 == 0
            ...
            fp = open("test2-only_lossvalue.txt", "a")
            println(fp, "$(it)-th loss = ",lossvalue/length(y0))
            close(fp)
        end
    end
end
...
savefig("comparison_Flux_onlyx.png")
